...
def getVideoUrl(url):
    ...
    # match = re.compile(r'.*content="(https://.*?datamm[^"]+)".*', re.DOTALL).findall(httpdata)
    match = re.compile(r'playerElement_id([0-9]+)',
                       re.DOTALL).findall(httpdata)
    if match:
        video_id = match[0]
        xmldata = getXmlFromUrl('https://tv.sme.sk/storm-sme-crd/'
                                'mmdata_get.asp?vp=2&id={0}'.format(video_id))
    ...
    match = re.compile(r'content="https://i.sme.sk/(datamm[^"]+)',
                       re.DOTALL).findall(httpdata)
    if match:
